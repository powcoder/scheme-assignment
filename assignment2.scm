...
; The deadline is Sept 23rd 23:59:59PM
...
(define q1-result 0) ; write your own code to calculate this
(display q1-result) ; don't change this line
...
; For example, a pair of eights beats a pair of fives (see example 1 below). But if two ranks tie, for example, both players have a pair of queens,
; then highest cards in each hand are compared (see example 4 below); if the highest cards tie then the next highest cards are compared, and so on.
...
; q2-data contains one-thousand random hands dealt to two players. Each string contains ten cards (separated by a single space): the first five are Player 1's cards and the last five are Player 2's cards.
...
(define q2-result 0) ; write your own code to calculate this
(display q2-result) ; don't change this line
(display " ") ; don't change this line
